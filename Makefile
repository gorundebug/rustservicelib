...
fmt:
	bash -c 'source scripts/dependency-proxy-env.sh; docker run --rm -v "$(CURDIR):/workspace" -w /workspace "$${DEPENDENCY_DOCKER_REGISTRY:-docker.io}/library/rust:1.98.1-bookworm" cargo fmt --all'
