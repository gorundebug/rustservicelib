...
where
    ...
    H: EndpointHandler<HandlerState, ReqT, ResR, T, R, E> + 'static,
{
    fn spawn_finalizer(&self, stream_id: String, pending: Arc<Pending<HandlerState, ReqT, ResR>>) {
        ...
        tokio::spawn(async move {
            ...
            let _operation = pending.operation.lock().await;
            let removed = {
            ...
            let result = if cancelled {
                    ...
                    .await
                {
                    Ok(response) => {
                        pending
                            .span
                            .in_scope(|| tracing::info!(event.name = "close_and_recv"));
                        let handled = handler
                            .handle_response(
                    ...
                    Err(error) => {
                        ...
                        pending.span.in_scope(|| {
                            tracing::error!(
                                event.name = "close_and_recv.error",
                                error = %error
                            )
                        });
                        Err(error)
    ...
    H: EndpointHandler<HandlerState, ReqT, ResR, T, R, E> + 'static,
{
    async fn consume(&self, context: MessageContext, value: Payload<T>) {
        ...
        drop(_operation);
        if created {
            self.spawn_finalizer(stream_id, pending);
        }
    }
}
