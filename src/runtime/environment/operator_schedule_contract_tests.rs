...
use crate::{
    ...
    runtime::{
        ...
        config::{
            CallSemantics, Config, LinkConfig, RuntimeConfig, RuntimeStreamConfig, StreamConfig,
        },
        stream::Stream,
    },
};
...
async fn split_dispatches_parallel_branch_before_awaiting_direct_branch() {
    ...
    tokio::time::timeout(
        ...
        source.emit(MessageContext::new(), Payload::new(42)),
    )
    .await
    .expect("direct branch must not prevent parallel branch dispatch");
    {
...
async fn cancelled_delay_releases_payload_and_cannot_emit_after_original_expiry() {
    for cancel in [false, true] {
        let environment = environment(
            RuntimeStreamConfig::Delay(StreamConfig::new(2, "Delay").into()),
            Vec::new(),
        );
        ...
        assert_eq!(
            *values.0.lock().unwrap(),
            if cancel { vec![] } else { vec![42] }
        );
