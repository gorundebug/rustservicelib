...
impl RuntimeEnvironment {
    ...
    pub(crate) async fn start_runtime_metrics(&self) -> RuntimeResult<()> {
        self.tokio_runtime_metrics.start(&self.metrics).await
    }

    pub(crate) async fn stop_runtime_metrics(&self) -> RuntimeResult<()> {
        self.tokio_runtime_metrics.stop().await
    }
