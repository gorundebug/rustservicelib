...
fn remote_parent() -> opentelemetry::Context {
    opentelemetry::Context::new().with_remote_span_context(SpanContext::new(
        TraceId::from(42_u128), SpanId::from(7_u64), TraceFlags::SAMPLED,
...
fn disabled_span_preserves_the_existing_propagation_context_even_when_sampled() {
    for sampled in [false, true] {
        ...
        let message = if sampled { message.enable_sampling() } else { message };
        let result = message.with_span_context(&tracing::Span::none());
        assert_eq!(result.open_telemetry_context().span().span_context(), parent.span().span_context());
    }
}
...
fn enabled_span_replaces_the_parent_only_for_a_sampled_message() {
    ...
    let subscriber = tracing_subscriber::registry().with(
        tracing_opentelemetry::layer().with_tracer(provider.tracer("context-test")),
    );
    tracing::subscriber::with_default(subscriber, || {
        ...
        for sampled in [false, true] {
            ...
            assert_eq!(result.open_telemetry_context().span().span_context(), expected.span().span_context());
        }
    });
}
...
fn filtered_operator_and_grpc_spans_preserve_the_propagation_context() {
    ...
    impl RuntimeStream for GroupedStream {
        fn id(&self) -> i32 { 1 }
        fn name(&self) -> String { "Reserve".to_owned() }
        fn environment(&self) -> &RuntimeEnvironment { &self.0 }
        fn tracing_labels(&self) -> (&str, &str, &str) { ("Reserve", "booking", "Inventory") }
    }
    let stream = GroupedStream(RuntimeEnvironment::with_telemetry(
        CallSemantics::FunctionCall,
        Arc::new(TestMetrics::new()), Arc::new(TestTracing::default()), Arc::new(TestLog::default()),
    ));
    assert!(stream.environment().tracing_enabled());
    tracing::subscriber::with_default(tracing::subscriber::NoSubscriber::default(), || {
        let parent = remote_parent();
        let context = super::MessageContext::new().enable_sampling().with_open_telemetry_context(parent.clone());
        let (operator_context, span) = stream.start_span(context.clone(), "stream.map");
        assert!(span.is_disabled());
        assert_eq!(operator_context.open_telemetry_context().span().span_context(), parent.span().span_context());
        let (grpc_context, span) = crate::datasink::grpc::start_output_span(context, &stream, "Reserve");
        assert!(span.is_disabled());
        assert_eq!(grpc_context.open_telemetry_context().span().span_context(), parent.span().span_context());
    });
}
...
fn trace_scopes_and_errors_are_lazy_and_do_not_leak_to_parent() {
    for enabled in [false, true] {
        ...
        tracing::subscriber::with_default(subscriber, || {
            ...
            let span = if enabled {
                tracing::info_span!("transport", error = tracing::field::Empty,
...
fn business_callback_runs_once_and_returns_its_value_with_or_without_tracing() {
    ...
    tracing::subscriber::with_default(subscriber, || {
        for enabled in [false, true] {
            ...
            let span = if enabled { tracing::info_span!("business") } else { tracing::Span::none() };
...
fn transport_scopes_and_errors_use_lazy_callsite_guards() {
    for (name, source) in [
        ...
        ("Custom source", include_str!("../datasource/localsource/custom.rs")),
        ("Custom sink", include_str!("../datasink/localsink/custom.rs")),
        ("Kafka source", include_str!("../datasource/kafka/rdkafka.rs")),
        ...
        ("gRPC unary", include_str!("../datasink/grpc/nostreaming.rs")),
        ("gRPC server streaming", include_str!("../datasink/grpc/serverstreaming.rs")),
        ("gRPC client streaming", include_str!("../datasink/grpc/clientstreaming.rs")),
        ("gRPC bidi", include_str!("../datasink/grpc/bidistreaming.rs")),
    ] {
        let compact: String = source.chars().filter(|ch| !ch.is_whitespace()).collect();
        assert!(!compact.contains(".in_scope("), "unguarded scope in {name}");
        assert!(!compact.contains("telemetry::record_span_error("), "unguarded error in {name}");
    }
}
