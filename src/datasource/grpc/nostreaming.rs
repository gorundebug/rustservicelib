    ...
    pub(super) async fn receive(&self, context: MessageContext) -> HandlerResult<ResR> {
        loop {
            ...
            tokio::select! {
                _ = &mut notified => {}
                _ = context.cancelled() => {
                    // Sending the response and cancelling the RPC may wake the
                    // select at the same time. Prefer the completed response,
                    // matching the canonical Go source boundary.
                    if let Some(value) = self.take() {
                        return Ok(value);
                    }
