...
async fn accepted_expiry(stop_before_cancel: bool, drop_owner: bool) {
    ...
    context.cancel();
    tokio::time::timeout(Duration::from_secs(2), completed.acquire())
        .await
        .expect("storage stop suppressed an already accepted expiry callback")
        .unwrap()
        .forget();
    ...
    .await
    .expect("completed expiry retained accepted input");
    assert_eq!(calls.load(Ordering::SeqCst), 2);
    if let Some(store) = retained_owner {
        assert_eq!(store.len().await, 0);
    }
}

#[tokio::test]
async fn accepted_context_expiry_completes() {
    accepted_expiry(false, false).await;
}

#[tokio::test]
async fn accepted_context_expiry_survives_storage_stop() {
    accepted_expiry(true, false).await;
}
...
async fn stopped_storage_preserves_join_completion_and_reuse() {
    for ttl in [Duration::ZERO, Duration::from_secs(3600)] {
        ...
        assert!(
            store.start(MessageContext::new()).await.is_err(),
            "stop still forbids restart"
        );
        assert!(
            store
                .join_value(
...
async fn configured_ttl_survives_stop_and_external_owner_drop() {
    ...
    assert!(
        store
            .join_value(MessageContext::new(), 7, 0, value, callback)
            .await
    );
    store.stop(MessageContext::new()).await;
    drop(store);
    tokio::time::timeout(Duration::from_secs(2), completed.acquire())
        .await
        .expect("configured TTL callback lost after stop/drop")
        .unwrap()
        .forget();
    tokio::time::timeout(Duration::from_secs(2), async {
        while weak.upgrade().is_some() {
    ...
    .await
    .expect("completed TTL retained accepted input");
    assert_eq!(calls.load(Ordering::SeqCst), 2);
}
