...
use tokio::{sync::{Semaphore, mpsc}, time::{Instant, advance, timeout}};
...
async fn ttl_includes_time_spent_in_the_initial_callback() {
    ...
    let input = {
        ...
        tokio::spawn(async move {
            store.join_value(MessageContext::new(), 7, 0, Arc::new(42_u32), callback).await
        })
    };
    entered.acquire().await.unwrap().forget();
    advance(ttl * 2).await;
    assert_eq!(calls.load(Ordering::SeqCst), 1, "expiry must serialize with the active callback");
    ...
    timeout(Duration::from_secs(1), expired.acquire()).await
        .expect("TTL was restarted after the initial callback").unwrap().forget();
...
async fn renewed_ttl_does_not_expire_at_the_original_deadline() {
    ...
    assert!(store.join_value(MessageContext::new(), 7, 1, Arc::new(20_u32), callback).await);
    let expired_at = timeout(ttl * 2, receiver.recv()).await.unwrap().unwrap();
    assert!(expired_at >= renewed_at + ttl, "original timer bypassed renewed TTL");
