use std::{
    future::Future,
    pin::Pin,
    sync::{Arc, Mutex, Weak, atomic::{AtomicBool, AtomicUsize, Ordering}},
    task::Poll,
    time::Duration,
};
...
use servicelib::{
    ...
    datasource::grpc::{
        ...
        make_grpc_no_streaming_endpoint_consumer, make_grpc_client_streaming_endpoint_consumer,
...
enum Phase { Begin, Eof, Read, TransportRead, Result, Normal }
...
impl Probe {
    fn record_task(&self) { self.tasks.lock().unwrap().push(tokio::task::try_id()); }
...
impl EndpointHandler<State, u32, u32, u32, u32, String> for Handler {
    async fn begin_request(&self, context: MessageContext, _: StreamContext<u32, u32, String>) -> HandlerResult<(MessageContext, State)> {
        self.0.record_task();
        ...
        if first {
            ...
            *self.0.lifetime.lock().unwrap() = Some(Arc::downgrade(&lifetime));
            if matches!(self.0.phase, Phase::Begin) { self.0.hold().await; }
        }
        Ok((context, State { first, _lifetime: lifetime }))
    }

    async fn consume_message(&self, context: MessageContext, _: StreamContext<u32, u32, String>, state: Arc<AsyncMutex<State>>, request: u32, result: Arc<ResultContext<State, u32, u32, u32, String>>, sender: Arc<dyn Sender<u32>>) -> HandlerResult<MessageContext> {
        self.0.record_task();
        let first = state.lock().await.first;
        if first { self.0.consumes.fetch_add(1, Ordering::SeqCst); }
    ...
    async fn get_message_id(&self, _: &MessageContext, _: &StreamContext<u32, u32, String>, _: Arc<AsyncMutex<State>>, _: &u32) -> String {
        "reply".into()
    }

    async fn eof(&self, _: MessageContext, _: StreamContext<u32, u32, String>, state: Arc<AsyncMutex<State>>) {
        self.0.record_task();
        if state.lock().await.first {
            if matches!(self.0.phase, Phase::Eof) { self.0.hold().await; }
            self.0.eofs.fetch_add(1, Ordering::SeqCst);
        }
    }

    async fn end_request(&self, _: MessageContext, _: StreamContext<u32, u32, String>, _: &HandlerResult, state: Arc<AsyncMutex<State>>) -> HandlerResult {
        self.0.record_task();
        if state.lock().await.first { self.0.ends.fetch_add(1, Ordering::SeqCst); }
        Ok(())
    }
}
...
fn fixture(mode: GrpcMethodType, with_result: bool, phase: Phase) -> (Invoke, Arc<Probe>) {
    ...
    if with_result {
        input.set_source(&Stream::<u32>::new(&StreamConfig::new(2, "result"), environment)).unwrap();
    }
    let probe = Arc::new(Probe {
        phase, begins: AtomicUsize::new(0), consumes: AtomicUsize::new(0), eofs: AtomicUsize::new(0), ends: AtomicUsize::new(0),
        entered: Semaphore::new(0), release: Semaphore::new(0), context: Mutex::new(None), lifetime: Mutex::new(None), tasks: Mutex::new(Vec::new()),
    });
    let first_read = Arc::new(AtomicBool::new(matches!(phase, Phase::Read | Phase::TransportRead)));
    ...
    let tail = move |context: MessageContext| {
        ...
        stream::poll_fn(move |cx| {
            if held {
                if !entered { probe.entered.add_permits(1); entered = true; }
                ...
                Poll::Pending
            } else { Poll::Ready(None::<HandlerResult<u32>>) }
        })
    };
    let invoke: Invoke = match mode {
        GrpcMethodType::NoStreaming => {
            let endpoint = make_grpc_no_streaming_endpoint_consumer(input, Handler(probe.clone())).unwrap();
        ...
        GrpcMethodType::ServerStreaming => {
            let endpoint = make_grpc_server_streaming_endpoint_consumer(input, Handler(probe.clone())).unwrap();
            Arc::new(move |context| { let endpoint = endpoint.clone(); Box::pin(async move { endpoint.handle(context, 1, Arc::new(Discard)).await }) })
        }
        GrpcMethodType::ClientStreaming => {
            let endpoint = make_grpc_client_streaming_endpoint_consumer(input, Handler(probe.clone())).unwrap();
            Arc::new(move |context| {
                let endpoint = endpoint.clone(); let tail = tail(context.clone());
                Box::pin(async move {
                    // The input borrows a local array: handle must not require a static stream.
                    let requests = [1_u32];
                    endpoint.handle(context, stream::iter(requests.iter().copied().map(Ok)).chain(tail)).await.map(|_| ())
                })
            })
        }
        GrpcMethodType::BidirectionalStreaming => {
            let endpoint = make_grpc_bidi_streaming_endpoint_consumer(input, Handler(probe.clone())).unwrap();
            Arc::new(move |context| {
                let endpoint = endpoint.clone(); let tail = tail(context.clone());
                Box::pin(async move {
                    let requests = [1_u32];
                    endpoint.handle(context, stream::iter(requests.iter().copied().map(Ok)).chain(tail), Arc::new(Discard)).await
                })
            })
        }
...
async fn abandoned_phase(mode: GrpcMethodType, with_result: bool, phase: Phase) {
    ...
    if !matches!(phase, Phase::Read) {
        ...
        assert_eq!(probe.ends.load(Ordering::SeqCst), 0, "EndRequest ran before the active phase ended");
        probe.release.add_permits(1);
    }
    tokio::time::timeout(Duration::from_secs(5), async {
        while weak.upgrade().is_some() { tokio::task::yield_now().await; }
    }).await.expect("phase cancellation retained handler state");
    assert_eq!(probe.ends.load(Ordering::SeqCst), 1);
    assert_eq!(probe.consumes.load(Ordering::SeqCst), usize::from(!matches!(phase, Phase::Begin)));
    assert_eq!(probe.eofs.load(Ordering::SeqCst), usize::from(matches!(phase, Phase::Eof)));
    tokio::time::timeout(Duration::from_secs(5), invoke(parent)).await.unwrap().unwrap();
}

#[tokio::test]
async fn abandoned_begin_and_eof_complete_before_finalization() {
    for mode in [GrpcMethodType::NoStreaming, GrpcMethodType::ClientStreaming, GrpcMethodType::ServerStreaming, GrpcMethodType::BidirectionalStreaming] {
        for with_result in [false, true] {
            for phase in [Phase::Begin, Phase::Eof] { abandoned_phase(mode, with_result, phase).await; }
        }
    }
}
...
async fn abandoned_inbound_wait_finalizes_without_waiting_for_another_message() {
    for mode in [GrpcMethodType::ClientStreaming, GrpcMethodType::BidirectionalStreaming] {
        for with_result in [false, true] { abandoned_phase(mode, with_result, Phase::Read).await; }
    }
}
...
async fn context_cancellation_alone_finalizes_each_source_phase() {
    for mode in [GrpcMethodType::NoStreaming, GrpcMethodType::ClientStreaming,
        GrpcMethodType::ServerStreaming, GrpcMethodType::BidirectionalStreaming] {
        for with_result in [false, true] {
            for phase in [Phase::Begin, Phase::Eof, Phase::Result] {
                ...
                if matches!(phase, Phase::Begin | Phase::Eof) {
                    assert!(weak.upgrade().is_some(), "cancellation dropped an active handler");
                    assert_eq!(probe.ends.load(Ordering::SeqCst), 0);
                    assert!(!task.is_finished(), "cancellation overtook an active handler");
                    probe.release.add_permits(1);
                }
                ...
                assert_eq!(probe.ends.load(Ordering::SeqCst), 1);
                assert!(weak.upgrade().is_none(), "completed request retained handler state");
...
async fn sources_without_result_stream_do_not_wait_for_done() {
    for mode in [GrpcMethodType::NoStreaming, GrpcMethodType::ClientStreaming,
        ...
        tokio::time::timeout(Duration::from_secs(5), invoke(MessageContext::new()))
            .await.expect("source without result stream waited for Done").unwrap();
        assert_eq!(probe.ends.load(Ordering::SeqCst), 1);
        assert!(probe.lifetime.lock().unwrap().as_ref().unwrap().upgrade().is_none());
    }
}
...
async fn cancelled_inbound_transport_finalizes_streaming_sources() {
    for mode in [GrpcMethodType::ClientStreaming, GrpcMethodType::BidirectionalStreaming] {
        for with_result in [false, true] {
            ...
            assert_eq!(probe.eofs.load(Ordering::SeqCst), 0, "transport error is not EOF");
            assert!(weak.upgrade().is_none(), "cancelled transport retained handler state");
            tokio::time::timeout(Duration::from_secs(5),
