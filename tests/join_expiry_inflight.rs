...
async fn check_expiry_callback_lifetime(stop_during_callback: bool) {
    ...
    assert!(store.join_value(MessageContext::new(), 7, 0, old_value, callback).await);
    ...
    if stop_during_callback {
        ...
        assert!(store.join_value(MessageContext::new(), 7, 0, Arc::new(20_u32), replacement).await);
        assert_eq!(store.len().await, 1);
    }
    assert!(old_lifetime.upgrade().is_some(), "active callback lost its input");
    assert!(finished.try_acquire().is_err(), "callback escaped its gate");
    release.add_permits(1);
    tokio::time::timeout(Duration::from_secs(1), finished.acquire())
        .await.expect("active callback was aborted").unwrap().forget();
    tokio::time::timeout(Duration::from_secs(1), async {
        while old_lifetime.upgrade().is_some() {
            tokio::task::yield_now().await;
        }
    }).await.expect("completed expiry task retained its old input");
    assert_eq!(calls.load(Ordering::SeqCst), 2);
    if stop_during_callback {
        assert_eq!(store.len().await, 0, "finished expiry must remove its group");
    }
    ...
    if !stop_during_callback {
        ...
        assert!(store.join_value(MessageContext::new(), 7, 1, Arc::new(30_u32), complete).await);
