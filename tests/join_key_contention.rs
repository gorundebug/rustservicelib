...
async fn waiting_value_survives_previous_join_completion() {
    ...
    let first = tokio::spawn(async move {
        first_store
            .join_value(MessageContext::new(), 7, 0, Arc::new(1_u32), first_handler)
            .await
    });
    ...
    let accepted = tokio::time::timeout(Duration::from_secs(2), &mut waiting)
        .await
        .unwrap();
    assert!(
        accepted,
        "waiting value was discarded after the previous item completed"
    );
...
async fn independent_join_key_progresses_while_callback_waits() {
    ...
    let second = tokio::time::timeout(
        ...
        store.join_value(MessageContext::new(), 8, 0, Arc::new(2_u32), handler),
    )
    .await;
    release.add_permits(1);
    assert!(first.await.unwrap());
    ...
    for round in 0..20 {
        ...
        for value in 0..64_u32 {
            ...
            calls.push(tokio::spawn(async move {
                barrier.wait().await;
                store
                    .join_value(MessageContext::new(), 7, 0, Arc::new(value), handler)
                    .await
            }));
        }
        let results =
            tokio::time::timeout(Duration::from_secs(5), futures::future::join_all(calls))
                .await
                .expect("same-key contenders did not finish");
        ...
        assert_eq!(
            store.len().await,
            0,
            "round {round}: completed entry remained"
        );
    }
}
