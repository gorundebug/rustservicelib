...
use servicelib::{
    ...
    api::GrpcMethodType,
    datasource::grpc::{
        ClientStreamingEndpointConsumer, ServerStreamingEndpointConsumer, BidiStreamingEndpointConsumer, EndpointHandler, HandlerResult,
        NoStreamingEndpointConsumer, ResultContext, Sender, StreamContext,
        make_grpc_client_streaming_endpoint_consumer,
        make_grpc_no_streaming_endpoint_consumer,
        make_grpc_server_streaming_endpoint_consumer, make_grpc_bidi_streaming_endpoint_consumer,
    },
    operators::InputStream,
    runtime::{
        common::Payload,
        config::{CallSemantics, GrpcDataConnectorConfig, GrpcEndpointConfig,
            InputStreamConfig, RuntimeConfig, StreamConfig},
        environment::RuntimeEnvironment,
        stream::Stream,
    },
};
use tokio::sync::{Mutex as AsyncMutex, Semaphore};
use tonic::{Request, Response, Status, codec::ProstCodec, codegen::{Body, BoxFuture, Service, StdError, http}};
...
impl EndpointHandler<bool, u32, u32, u32, u32, String> for Handler {
    ...
    async fn consume_message(&self, context: MessageContext, _: StreamContext<u32, u32, String>, state: Arc<AsyncMutex<bool>>, request: u32, result: Arc<Results>, sender: Arc<dyn Sender<u32>>) -> HandlerResult<MessageContext> {
    ...
    async fn get_message_id(&self, _: &MessageContext, _: &StreamContext<u32, u32, String>, _: Arc<AsyncMutex<bool>>, _: &u32) -> String {
        "reply".into()
    }

    async fn eof(&self, _: MessageContext, _: StreamContext<u32, u32, String>, _: Arc<AsyncMutex<bool>>) {}

    async fn end_request(&self, _: MessageContext, _: StreamContext<u32, u32, String>, _: &HandlerResult, state: Arc<AsyncMutex<bool>>) -> HandlerResult {
        if *state.lock().await {
    ...
    fn call(&mut self, request: Request<u32>) -> Self::Future {
        ...
        Box::pin(async move {
            ...
            tokio::spawn(async move {
                if let Err(error) = endpoint.handle(task_context, request.into_inner(), Arc::new(ResponseSender(sender.clone()))).await {
...
impl tonic::server::StreamingService<u32> for BidiMethod {
    ...
    fn call(&mut self, request: Request<tonic::Streaming<u32>>) -> Self::Future {
        ...
        Box::pin(async move {
            let context = MessageContext::from_tonic_request_with_tracing(&request, false);
            let task_context = context.clone();
            let requests = request.into_inner().map(|result| result.map_err(|error| Box::new(error) as servicelib::datasource::grpc::HandlerError));
            let (sender, receiver) = tokio::sync::mpsc::channel(16);
            tokio::spawn(async move {
                if let Err(error) = endpoint.handle(task_context, requests, Arc::new(ResponseSender(sender.clone()))).await {
                    let _ = sender.send(Err(Status::internal(error.to_string()))).await;
                }
            });
            Ok(Response::new(ResponseStream { receiver, context }))
...
impl tonic::server::UnaryService<u32> for UnaryMethod {
    ...
    fn call(&mut self, request: Request<u32>) -> Self::Future {
        ...
        Box::pin(async move {
            let context = MessageContext::from_tonic_request_with_tracing(&request, false);
            endpoint.handle(context, request.into_inner()).await
                .map(Response::new).map_err(|error| Status::internal(error.to_string()))
        })
    }
}
...
impl tonic::server::ClientStreamingService<u32> for ClientMethod {
    ...
    fn call(&mut self, request: Request<tonic::Streaming<u32>>) -> Self::Future {
        ...
        Box::pin(async move {
            let context = MessageContext::from_tonic_request_with_tracing(&request, false);
            let requests = request.into_inner().map(|result| result.map_err(|error| Box::new(error) as servicelib::datasource::grpc::HandlerError));
            endpoint.handle(context, requests).await
                .map(Response::new).map_err(|error| Status::internal(error.to_string()))
        })
    }
}
    ...
    fn call(&mut self, request: http::Request<B>) -> Self::Future {
        ...
        Box::pin(async move {
            ...
            Ok(match endpoint {
                ...
                Endpoint::Client(endpoint) => grpc.client_streaming(ClientMethod(endpoint), request).await,
                Endpoint::Server(endpoint) => grpc.server_streaming(ServerMethod(endpoint), request).await,
...
async fn call(channel: tonic::transport::Channel, streaming: bool) -> Result<Response<u32>, Box<Status>> {
    let mut client = tonic::client::Grpc::new(channel);
    client.ready().await.map_err(|error| Status::unknown(error.to_string()))?;
    let path = http::uri::PathAndQuery::from_static("/parity.Lifetime/Call");
    if streaming {
        let mut request = Request::new(tokio_stream::iter([42_u32]));
        request.metadata_mut().insert("x-stream-id", "network-rpc".parse().unwrap());
        client.client_streaming(request, path, ProstCodec::<u32, u32>::default()).await.map_err(Box::new)
    } else {
        let mut request = Request::new(42_u32);
        request.metadata_mut().insert("x-stream-id", "network-rpc".parse().unwrap());
        client.unary(request, path, ProstCodec::<u32, u32>::default()).await.map_err(Box::new)
    }
}

async fn gate(semaphore: &Semaphore) {
    tokio::time::timeout(Duration::from_secs(5), semaphore.acquire()).await.unwrap().unwrap().forget();
}
...
async fn disconnected_rpc(streaming: bool, with_result: bool) {
    ...
    let mode = if streaming { GrpcMethodType::ClientStreaming } else { GrpcMethodType::NoStreaming };
    ...
    if with_result { input.set_source(&results).unwrap(); }
    let probe = Arc::new(Probe {
        begins: AtomicUsize::new(0), consumed: AtomicUsize::new(0), original_ended: AtomicUsize::new(0),
        callbacks: Arc::new(AtomicUsize::new(0)), entered: Semaphore::new(0), release_consume: Semaphore::new(0),
        ending: Semaphore::new(0), release_end: Semaphore::new(0), context: Mutex::new(None), result: Mutex::new(None),
    });
    let endpoint = if streaming {
        Endpoint::Client(make_grpc_client_streaming_endpoint_consumer(input, Handler(probe.clone())).unwrap())
    } else {
        Endpoint::Unary(make_grpc_no_streaming_endpoint_consumer(input, Handler(probe.clone())).unwrap())
    };
    let listener = tokio::net::TcpListener::bind("127.0.0.1:0").await.unwrap();
    let address = listener.local_addr().unwrap();
    let _server = ServerTask(tokio::spawn(async move {
        tonic::transport::Server::builder().add_service(TestServer(endpoint))
    ...
    tokio::time::timeout(Duration::from_secs(5), context.cancelled()).await
    ...
    tokio::time::timeout(Duration::from_secs(5), results.emit(context.clone(), Payload::new(7))).await.unwrap();
    let callbacks = usize::from(with_result);
    assert_eq!(probe.callbacks.load(Ordering::SeqCst), callbacks, "cancellation closed callbacks before ConsumeMessage completed");

    probe.release_consume.add_permits(1);
    gate(&probe.ending).await;
    assert_eq!(probe.consumed.load(Ordering::SeqCst), 1, "accepted handler did not complete");
    tokio::time::timeout(Duration::from_secs(5), results.emit(context, Payload::new(8))).await.unwrap();
    assert_eq!(probe.callbacks.load(Ordering::SeqCst), callbacks, "EndRequest admitted a late result");
    let duplicate = tokio::time::timeout(Duration::from_secs(5), call(channel.clone(), streaming)).await.unwrap();
    assert!(duplicate.unwrap_err().message().contains("duplicate"));
    assert_eq!(probe.original_ended.load(Ordering::SeqCst), 0);
    probe.release_end.add_permits(1);
    tokio::time::timeout(Duration::from_secs(5), async {
        while result.upgrade().is_some() { tokio::task::yield_now().await; }
    }).await.expect("wire-cancelled request retained its result context");
    assert_eq!(probe.original_ended.load(Ordering::SeqCst), 1);
    let response = tokio::time::timeout(Duration::from_secs(5), call(channel, streaming)).await.unwrap().unwrap();
...
async fn unary_wire_cancellation_preserves_handler_and_finalizer() {
    for with_result in [false, true] { disconnected_rpc(false, with_result).await; }
}

#[tokio::test]
async fn client_stream_wire_cancellation_preserves_handler_and_finalizer() {
    for with_result in [false, true] { disconnected_rpc(true, with_result).await; }
}

async fn open_response(channel: tonic::transport::Channel, bidi: bool) -> Result<Response<tonic::Streaming<u32>>, Box<Status>> {
    let mut client = tonic::client::Grpc::new(channel);
    client.ready().await.map_err(|error| Status::unknown(error.to_string()))?;
    let path = http::uri::PathAndQuery::from_static("/parity.Lifetime/Call");
    if bidi {
        let mut request = Request::new(tokio_stream::iter([42_u32]));
        request.metadata_mut().insert("x-stream-id", "network-rpc".parse().unwrap());
        client.streaming(request, path, ProstCodec::<u32, u32>::default()).await.map_err(Box::new)
    } else {
        let mut request = Request::new(42_u32);
        request.metadata_mut().insert("x-stream-id", "network-rpc".parse().unwrap());
        client.server_streaming(request, path, ProstCodec::<u32, u32>::default()).await.map_err(Box::new)
    }
}
...
async fn dropped_response(bidi: bool, with_result: bool) {
    let environment = RuntimeEnvironment::default();
    let config = InputStreamConfig { stream: StreamConfig::new(1, "input"), endpoint_id: 4 };
    let mode = if bidi { GrpcMethodType::BidirectionalStreaming } else { GrpcMethodType::ServerStreaming };
    environment.publish_runtime_config(Arc::new(RuntimeConfig::from_parts(
        CallSemantics::FunctionCall, [], [config.clone().into()], [],
        [GrpcDataConnectorConfig { id: 10, name: "grpc".into(), address: "http://localhost".into(), connections_count: 1 }.into()],
        [GrpcEndpointConfig { id: 4, name: "request".into(), id_data_connector: 10, tracing_enabled: false, grpc_method_type: mode }.into()], [],
    ...
    let endpoint = if bidi {
        Endpoint::Bidi(make_grpc_bidi_streaming_endpoint_consumer(input, Handler(probe.clone())).unwrap())
    } else {
        Endpoint::Server(make_grpc_server_streaming_endpoint_consumer(input, Handler(probe.clone())).unwrap())
    };
    let listener = tokio::net::TcpListener::bind("127.0.0.1:0").await.unwrap();
    let address = listener.local_addr().unwrap();
    let _server = ServerTask(tokio::spawn(async move {
    ...
    let channel = tonic::transport::Endpoint::from_shared(format!("http://{address}")).unwrap().connect().await.unwrap();
    let response = tokio::time::timeout(Duration::from_secs(5), open_response(channel.clone(), bidi)).await.unwrap().unwrap();
    gate(&probe.entered).await;
    let context = probe.context.lock().unwrap().clone().unwrap();
    ...
    }).await.expect("dropped response retained the original request");
    assert_eq!(probe.original_ended.load(Ordering::SeqCst), 1);
...
async fn server_stream_response_drop_preserves_handler_and_finalizer() {
    for with_result in [false, true] { dropped_response(false, with_result).await; }
}

#[tokio::test]
async fn bidi_response_drop_preserves_handler_and_finalizer() {
    for with_result in [false, true] { dropped_response(true, with_result).await; }
}
