...
use servicelib::{
    ...
    api::HTTPMethodType,
    datasource::http::{
        AxumDataSource, EndpointHandler, HandlerData, HandlerError, HandlerResult, ResultCallback,
        ResultContext,
    },
    ...
    runtime::{
        common::Payload,
        config::{
            CallSemantics, HttpDataConnectorConfig, HttpEndpointConfig, InputStreamConfig,
...
impl EndpointHandler<(), (), (), u32, u32, String> for Handler {
    ...
    async fn consume_message(
        ...
        _: Arc<AsyncMutex<()>>,
        data: HandlerData,
        result: Arc<ResultContext<(), (), (), u32, u32, String>>,
    ) -> HandlerResult {
        let probe = self.0.clone();
        result.set_result_callback(
            "reply",
            ResultCallback::new(move |_, _, _, _, _| {
...
fn fixture() -> (Arc<AxumDataSource>, axum::Router, Stream<u32>, Arc<Probe>) {
    ...
    let connector_config = HttpDataConnectorConfig {
        id: 10,
        name: "http".into(),
        host: "127.0.0.1".into(),
        port: 9090,
    ...
    source
        .add_endpoint(input, endpoint_config, Handler(probe.clone()))
        .unwrap();
...
async fn aborted_http_request_rejects_late_results_and_releases_its_id() {
    ...
    tokio::time::timeout(Duration::from_secs(5), probe.ended.acquire())
        .await
        .unwrap()
        .unwrap()
        .forget();
    ...
    tokio::time::timeout(
        ...
        results.emit(context, Payload::new(7)),
    )
    .await
    .unwrap();
    let request = Request::builder()
...
async fn disconnected_http_client_rejects_late_results_and_releases_its_id() {
    ...
    let cancelled = tokio::time::timeout(Duration::from_secs(2), context.cancelled())
        .await
        .is_ok();
...
async fn cancelled_request_in_end_request() -> (Arc<AxumDataSource>, Arc<Probe>) {
    ...
    first.abort();
    assert!(first.await.unwrap_err().is_cancelled());
    tokio::time::timeout(Duration::from_secs(5), probe.end_entered.acquire())
        .await
        .unwrap()
        .unwrap()
        .forget();
...
async fn datasource_stop_drains_disconnected_request_handlers() {
    ...
    let mut stop = tokio::spawn(async move {
        source
            .stop(MessageContext::with_timeout(Duration::from_secs(5)))
            .await
    });
    ...
    let finished = match early {
        ...
        Err(_) => tokio::time::timeout(Duration::from_secs(5), stop)
            .await
            .unwrap(),
    };
    ...
    assert!(
        !returned_early,
        "datasource stop ignored a disconnected request's EndRequest"
    );
}
...
async fn datasource_stop_deadline_does_not_drop_disconnected_request_handlers() {
    ...
    let stopped = tokio::time::timeout(
        ...
        source.stop(MessageContext::with_timeout(Duration::from_millis(50))),
    )
    .await;
    let ended_before_release = probe.ended.try_acquire().is_ok();
    probe.release_end.add_permits(1);
    stopped
        .expect("datasource ignored its shutdown deadline")
        .unwrap();
    assert!(
        !ended_before_release,
        "blocked handler unexpectedly completed"
    );
    tokio::time::timeout(Duration::from_secs(5), probe.ended.acquire())
        .await
        .unwrap()
