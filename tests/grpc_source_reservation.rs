...
impl EndpointHandler<bool, u32, u32, u32, u32, String> for SharedHandler {
    ...
    async fn consume_message(
        ...
        request: u32,
        result: Arc<ResultContext<bool, u32, u32, u32, String>>,
        ...
        *self.result_context.lock().unwrap() = Some(Arc::downgrade(&result));
        ...
        if first && matches!(self.phase, Phase::Consume) {
            self.hold().await;
        }
    ...
    async fn end_request(
        ...
        if first && matches!(self.phase, Phase::End) {
            self.hold().await;
        }
...
fn fixture(mode: GrpcMethodType, with_result: bool, phase: Phase) -> (Invoke, Arc<Handler>) {
    ...
    let handler = Arc::new(Handler {
        ...
        capture_result: AtomicBool::new(false),
        result_context: std::sync::Mutex::new(None),
    });
    let invoke: Invoke = match mode {
        GrpcMethodType::NoStreaming => {
            let endpoint =
                make_grpc_no_streaming_endpoint_consumer(input, SharedHandler(handler.clone()))
                    .unwrap();
        ...
        GrpcMethodType::ClientStreaming => {
            let endpoint =
                make_grpc_client_streaming_endpoint_consumer(input, SharedHandler(handler.clone()))
                    .unwrap();
            Arc::new(move |context| {
                let endpoint = endpoint.clone();
                Box::pin(async move {
        ...
        GrpcMethodType::ServerStreaming => {
            let endpoint =
                make_grpc_server_streaming_endpoint_consumer(input, SharedHandler(handler.clone()))
                    .unwrap();
            Arc::new(move |context| {
                let endpoint = endpoint.clone();
                Box::pin(async move { endpoint.handle(context, 1, Arc::new(DiscardSender)).await })
            })
        }
        GrpcMethodType::BidirectionalStreaming => {
            let endpoint =
                make_grpc_bidi_streaming_endpoint_consumer(input, SharedHandler(handler.clone()))
                    .unwrap();
            Arc::new(move |context| {
                let endpoint = endpoint.clone();
                Box::pin(async move {
                    endpoint
                        .handle(context, stream::iter([Ok(1)]), Arc::new(DiscardSender))
                        .await
                })
            })
        }
...
async fn check(mode: GrpcMethodType, with_result: bool, phase: Phase) {
    ...
    tokio::time::timeout(Duration::from_secs(5), handler.entered.acquire())
        .await
        .unwrap()
        .unwrap()
        .forget();
    ...
    let other = tokio::time::timeout(
        ...
        invoke(MessageContext::new().with_stream_id("other")),
    )
    .await;
    let duplicates = tokio::time::timeout(
        Duration::from_secs(5),
        join_all((0..8).map(|_| invoke(context.clone()))),
    )
    .await;
    ...
    assert_eq!(
        consumed_before_release, 2,
        "duplicate requests reached business handlers"
    );
    assert_eq!(
        handler.callbacks.load(Ordering::SeqCst),
        if with_result { 2 } else { 0 },
        "late results reached a closing request"
    );
    // A completed invocation must not reserve the ID forever.
    tokio::time::timeout(Duration::from_secs(5), invoke(context))
        .await
        .unwrap()
        .unwrap();
...
macro_rules! reservation_test {
    ($name:ident, $mode:ident, $result:expr, $phase:ident) => {
        #[tokio::test]
        async fn $name() {
            check(GrpcMethodType::$mode, $result, Phase::$phase).await;
        }
    };
}
...
async fn check_callback_release(mode: GrpcMethodType, with_result: bool) {
    ...
    tokio::time::timeout(
        Duration::from_secs(5),
        invoke(MessageContext::new().with_stream_id("callback-owner")),
    )
    .await
    .unwrap()
    .unwrap();
    ...
    assert!(
        result_context.upgrade().is_none(),
        "completed request retained a callback capturing its own ResultContext"
    );
}
...
macro_rules! callback_release_test {
    ($name:ident, $mode:ident, $result:expr) => {
        #[tokio::test]
        async fn $name() {
            check_callback_release(GrpcMethodType::$mode, $result).await;
        }
    };
}
...
async fn check_late_callback_registration(mode: GrpcMethodType, with_result: bool) {
    ...
    assert!(
        ...
        "late registration revived a finished request's callback cycle"
    );
}
...
async fn check_abandoned_source_request(mode: GrpcMethodType, with_result: bool) {
    for phase in [Phase::Consume, Phase::End] {
        ...
        tokio::time::timeout(Duration::from_secs(2), handler.entered.acquire())
            ...
            .unwrap()
            .forget();
        let result_lifetime = handler
            .result_context
            .lock()
            .unwrap()
            .as_ref()
            .expect("admitted request has a result context")
            .clone();
        active.abort();
        assert!(active.await.unwrap_err().is_cancelled());
        assert!(
            !parent.is_cancelled(),
            "request cancellation escaped into its caller"
        );
        let duplicate = tokio::time::timeout(Duration::from_secs(2), invoke(parent.clone()))
            .await
            .unwrap();
        ...
        .await
        .expect("abandoned request did not retire after its handler was released");
        assert_eq!(
            handler.consumed.load(Ordering::SeqCst),
            1,
            "admitted handler was dropped"
        );
        assert_eq!(
            handler.ended.load(Ordering::SeqCst),
            2,
            "original and duplicate must each finalize once"
        );
        let retry = tokio::time::timeout(
            Duration::from_secs(2),
            invoke(MessageContext::new().with_stream_id("abandoned")),
        )
        .await
        .unwrap();
        assert!(
            retry.is_ok(),
            "abandoned handler kept its stream ID reserved: {retry:?}"
        );
    }
}
