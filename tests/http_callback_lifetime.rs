...
fn callback_capturing_result(
    result: Arc<HttpResult>,
) -> ResultCallback<(), (), (), u32, u32, String> {
    ResultCallback::new(
        move |_context, _stream, _state: Arc<AsyncMutex<()>>, _value: Payload<u32>, _data| {
...
impl EndpointHandler<(), (), (), u32, u32, String> for Handler {
    ...
    async fn consume_message(
        ...
        *self.0.weak.lock().unwrap() = Some(Arc::downgrade(&result));
        ...
        if self.0.fail {
            Err("intentional consume failure".into())
        } else {
            Ok(())
        }
    }
    ...
    async fn end_request(
        ...
        data.set_status(if result.is_err() {
...
async fn complete_request(probe: Arc<Probe>) {
    let input_config = InputStreamConfig {
        stream: StreamConfig::new(1, "request"),
        endpoint_id: 4,
    };
    ...
    if probe.with_result {
        let result_stream =
            Stream::<u32>::new(&StreamConfig::new(2, "result"), environment.clone());
        input.set_source(&result_stream).unwrap();
    }
    let source = AxumDataSource::new(environment, &connector);
    source
        .add_endpoint(input, endpoint, Handler(probe.clone()))
        .unwrap();
    let response = source
        .router()
        .oneshot(Request::post("/request").body(Body::empty()).unwrap())
        .await
        .unwrap();
    let expected = if probe.fail || probe.expire {
        StatusCode::INTERNAL_SERVER_ERROR
    } else {
        StatusCode::OK
    };
