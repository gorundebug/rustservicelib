...
use servicelib::{
    ...
    datasink::kafka::{
        ...
        StreamContext, make_endpoint_consumer, make_rdkafka_kafka_endpoint_consumer,
    },
    runtime::{
...
async fn kafka_sink_connector_owns_endpoints_and_obeys_lifecycle() {
    ...
    let data_sink = RdkafkaKafkaDataSink::new(&connector_config).unwrap();
...
async fn kafka_sink_preserves_lifecycle_partition_and_delivery_result() {
    ...
    assert!(metrics.contains(
        r#"datasink_endpoint_messages_total{connector="kafka",endpoint="orders topic",protocol="kafka"} 1"#
    ));
}
