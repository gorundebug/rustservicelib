use std::{future::Future, pin::Pin, sync::{Arc, Mutex, Weak, atomic::{AtomicUsize, Ordering}}, time::Duration};

use async_trait::async_trait;
use futures::stream;
use servicelib::{
    MessageContext, api::GrpcMethodType,
...
impl EndpointHandler<bool, u32, u32, u32, u32, String> for Handler {
    ...
    async fn consume_message(&self, context: MessageContext, _: StreamContext<u32, u32, String>, state: Arc<AsyncMutex<bool>>, request: u32, result: Arc<Results>, sender: Arc<dyn Sender<u32>>) -> HandlerResult<MessageContext> {
        if *state.lock().await {
            ...
            *self.0.result.lock().unwrap() = Some(Arc::downgrade(&result));
            ...
            result.set_result_callback("reply", Arc::new(move |context, _, _, _, sender| {
    ...
    async fn get_message_id(&self, _: &MessageContext, _: &StreamContext<u32, u32, String>, _: Arc<AsyncMutex<bool>>, _: &u32) -> String { "reply".into() }
...
fn fixture(mode: GrpcMethodType) -> (Invoke, Stream<u32>, Arc<Probe>) {
    ...
    let probe = Arc::new(Probe {
        begins: AtomicUsize::new(0), callbacks: AtomicUsize::new(0), ends: AtomicUsize::new(0),
        consumed: Semaphore::new(0), callback_entered: Semaphore::new(0), release_callback: Semaphore::new(0),
        end_entered: Semaphore::new(0), release_end: Semaphore::new(0), context: Mutex::new(None), result: Mutex::new(None),
    });
    let invoke: Invoke = match mode {
        ...
        GrpcMethodType::BidirectionalStreaming => {
            ...
            Arc::new(move |context| { let endpoint = endpoint.clone(); Box::pin(async move { endpoint.handle(context, stream::iter([Ok(1)]), Arc::new(Discard)).await }) })
        }
...
async fn check(mode: GrpcMethodType, active_callback: bool) {
    ...
    if active_callback {
        assert!(tokio::time::timeout(Duration::from_millis(20), probe.end_entered.acquire()).await.is_err(), "EndRequest raced an active callback");
        assert!(weak.upgrade().is_some(), "active callback lost its request state");
    }
    let duplicate = tokio::time::timeout(Duration::from_secs(5), invoke(MessageContext::new().with_stream_id("drain"))).await.unwrap();
    assert!(duplicate.unwrap_err().to_string().contains("duplicate"));
    if let Some(callback) = callback {
        probe.release_callback.add_permits(1);
        tokio::time::timeout(Duration::from_secs(5), callback).await.unwrap().unwrap();
    }
    gate(&probe.end_entered).await;
    tokio::time::timeout(Duration::from_secs(5), results.emit(context, Payload::new(8))).await.unwrap();
    assert_eq!(probe.callbacks.load(Ordering::SeqCst), usize::from(active_callback));
    probe.release_end.add_permits(1);
    tokio::time::timeout(Duration::from_secs(5), async {
        while weak.upgrade().is_some() { tokio::task::yield_now().await; }
    }).await.expect("finished callback/request cycle was retained");
    assert_eq!(probe.ends.load(Ordering::SeqCst), 1);
    tokio::time::timeout(Duration::from_secs(5), invoke(MessageContext::new().with_stream_id("drain"))).await.unwrap().unwrap();
}
