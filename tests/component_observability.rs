...
use servicelib::{MessageContext, Payload, Stream, operators::MapFunction,
    runtime::{collector::Collector, common::RuntimeStream,
        config::{CallSemantics, RuntimeConfig, RuntimeStreamConfig, StreamConfig},
        environment::RuntimeEnvironment, testlog::TestLog, testmetrics::TestMetrics,
...
impl MapFunction<u32, u32> for Count {
    async fn map(&self, _context: MessageContext, _stream: &dyn RuntimeStream,
        _value: &u32, _out: &Collector<u32>) {
...
fn component_config_is_optional_and_round_trips_without_instance_identity() {
    ...
    assert!(!serde_json::to_value(&plain).unwrap().as_object().unwrap().contains_key("component"));
...
async fn receiving_grouping_is_used_on_existing_counter_and_operator_spans() {
    ...
    let environment = RuntimeEnvironment::with_telemetry(CallSemantics::FunctionCall,
        metrics.clone(),Arc::new(traces.clone()),Arc::new(TestLog::default()));
    let source_config = StreamConfig::new(1,"input").with_pipeline("entry").with_component("Request");
    let target_config = StreamConfig::new(2,"price").with_pipeline("pricing").with_component("Customer Pricing");
    environment.publish_runtime_config(Arc::new(RuntimeConfig::from_parts(
        CallSemantics::FunctionCall, [], [RuntimeStreamConfig::Plain(source_config.clone()),
            RuntimeStreamConfig::Map(target_config.clone().into())], [], [], [], [],
    ).unwrap()));
    let calls = Arc::new(AtomicUsize::new(0));
    let source = Stream::<u32>::new(&source_config,environment);
    let _mapped = source.map::<u32,_>(&target_config.into(),Count(calls.clone())).unwrap();
    ...
    source.emit(MessageContext::new().enable_sampling(),Payload::new(2)).await;
    drop(guard);
    ...
    for span in spans {
        ...
        assert_eq!(span.fields["component"].trim_matches('"'),"Customer Pricing");
        assert!(!span.fields.contains_key("component_instance"));
    }
    let text = metrics.prometheus();
    let sample = text.lines().find(|line| line.starts_with("stream_messages_total{")).unwrap();
...
fn operator_labels_are_borrowed_from_the_stream_not_reloaded_per_span() {
    ...
    let environment = RuntimeEnvironment::with_telemetry(
        CallSemantics::FunctionCall, Arc::new(TestMetrics::new()),
        Arc::new(traces.clone()), Arc::new(TestLog::default()),
    );
    let config = StreamConfig::new(1, "price").with_pipeline("pricing")
        .with_component("Customer Pricing");
    environment.publish_runtime_config(Arc::new(RuntimeConfig::from_parts(
        CallSemantics::FunctionCall, [], [RuntimeStreamConfig::Plain(config.clone())],
        [], [], [], [],
    ).unwrap()));
    ...
    assert!(std::ptr::eq(labels.1.as_ptr(), stream.tracing_labels().1.as_ptr()));
    ...
    assert_eq!(spans[0].fields["component"].trim_matches('"'), "Customer Pricing");
}
