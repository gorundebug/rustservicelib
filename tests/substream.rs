use std::{sync::{Arc, Mutex, OnceLock, atomic::{AtomicUsize, Ordering}}, time::Duration};

use async_trait::async_trait;
use servicelib::{MessageContext, Payload, SubStream, SubStreamCollector, SubStreamCollectorFunc,
    operators::MapFunction,
    runtime::{collector::Collector, common::RuntimeStream,
        config::{CallSemantics, MapStreamConfig, RuntimeConfig, RuntimeStreamConfig, StreamConfig, SubStreamConfig},
        environment::{RuntimeEnvironment, RuntimeError}, stream::Stream},
};
...
fn environment() -> (RuntimeEnvironment, SubStreamConfig, MapStreamConfig) {
    ...
    environment.publish_runtime_config(Arc::new(RuntimeConfig::from_parts(
        ...
        [RuntimeStreamConfig::from(entry.clone()), RuntimeStreamConfig::from(output.clone())],
        [], [], [], [],
    ).unwrap()));
...
impl MapFunction<i32, i32> for Echo {
    async fn map(&self, context: MessageContext, _: &dyn RuntimeStream, value: &i32, out: &Collector<i32>) {
...
async fn concurrent_calls_share_graph_not_results() {
    ...
    for value in 0..100 {
        ...
        tasks.spawn(async move {
            ...
            entry.consume(context, value, capture.clone()).await.unwrap();
...
impl MapFunction<i32, i32> for Multiple {
    async fn map(&self, context: MessageContext, _: &dyn RuntimeStream, value: &i32, out: &Collector<i32>) {
        for increment in 0..3 { out.collect(context.clone(), *value + increment).await; }
    }
}
...
async fn collector_controls_completion_and_later_results_are_dropped() {
    ...
    let collector = Arc::new(SubStreamCollectorFunc(move |_: MessageContext, value: Payload<i32>| {
        let mut values = collected.lock().unwrap();
        values.push(*value);
        let complete = values.len() == 2;
        async move { complete }
    }));
    entry.consume(MessageContext::new(), 40, collector).await.unwrap();
...
impl MapFunction<i32, i32> for Recurse {
    async fn map(&self, context: MessageContext, _: &dyn RuntimeStream, value: &i32, out: &Collector<i32>) {
        let result = if *value > 0 {
            let capture = Arc::new(Capture::default());
            self.0.get().unwrap().consume(context.clone(), *value - 1, capture.clone()).await.unwrap();
...
async fn recursive_calls_of_the_same_entry_restore_outer_results() {
    ...
    let capture = Arc::new(Capture::default());
    entry.consume(MessageContext::new().with_stream_id("parent"), 5, capture.clone()).await.unwrap();
...
async fn collector_can_call_the_same_substream() {
    ...
    let collector = Arc::new(SubStreamCollectorFunc(move |context: MessageContext, value: Payload<i32>| {
        let entry = nested_entry.clone();
        async move {
            let capture = Arc::new(Capture::default());
            entry.consume(context, *value + 1, capture.clone()).await.unwrap();
            assert_eq!(*capture.0.lock().unwrap(), vec![43]);
            true
        }
    }));
    entry.consume(MessageContext::new().with_stream_id("parent"), 42, collector).await.unwrap();
}
...
impl MapFunction<i32, i32> for Hold {
    async fn map(&self, context: MessageContext, _: &dyn RuntimeStream, value: &i32, _: &Collector<i32>) {
...
async fn cancellation_isolates_siblings_and_releases_retained_callbacks() {
    ...
    let first = tokio::spawn({
        let entry = entry.clone(); let context = cancelled.clone();
        async move { entry.consume(context, 1, capture1).await }
    });
    entered.notified().await;
    let capture2 = Arc::new(Capture::default());
    let second = tokio::spawn({
        let entry = entry.clone(); let capture = capture2.clone();
        async move { entry.consume(parent, 2, capture).await }
    });
    entered.notified().await;
    cancelled.cancel();
    assert!(matches!(first.await.unwrap(), Err(RuntimeError::ContextCancelled)));
    assert!(retained.upgrade().is_none());
    let contexts = contexts.lock().unwrap().clone();
    for (value, context) in contexts { output.emit(context, Payload::new(value)).await; }
...
async fn deadline_finishes_call_without_results() {
    ...
    let result = entry.consume(MessageContext::with_timeout(Duration::from_secs(1)), 1, Arc::new(Capture::default())).await;
    assert!(matches!(result, Err(RuntimeError::ContextCancelled)));
}

struct BlockingCollector { entered: Arc<Notify>, release: Arc<Notify>, completed: Arc<AtomicUsize> }
...
async fn cancellation_drains_an_active_direct_collector() {
    ...
    let task = tokio::spawn({
        let context = context.clone();
        let collector = Arc::new(BlockingCollector { entered: entered.clone(), release: release.clone(), completed: completed.clone() });
...
async fn aborted_caller_drops_callback_even_if_body_keeps_context() {
    ...
    let task = tokio::spawn(async move { entry.consume(MessageContext::new().with_stream_id("parent"), 1, capture).await });
    entered.notified().await;
    task.abort();
...
fn validates_body_result_and_service_ownership() {
    ...
    environment.build_runtime_streams().unwrap();
    assert_eq!(RuntimeStreamConfig::from(entry_config).transformation_type(), servicelib::api::TransformationType::SubStream);
}
