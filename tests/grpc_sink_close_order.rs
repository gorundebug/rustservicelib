...
use servicelib::{MessageContext, Payload, Stream,
    api::GrpcMethodType,
    datasink::grpc::{ClientStreamingCall, ClientStreamingClientFunction, EndpointHandler,
        HandlerResult, ResultContext, Sender, StreamContext, make_grpc_client_streaming_endpoint_consumer},
    runtime::{config::{CallSemantics, GrpcDataConnectorConfig, GrpcEndpointConfig, RuntimeConfig,
...
impl ClientStreamingCall<u32, u32> for Call {
    async fn send(&self, _: u32) -> HandlerResult { Ok(()) }
...
impl EndpointHandler<(), u32, u32, u32, u32, String> for Handler {
    async fn begin_request(&self, context: MessageContext, _: StreamContext<u32, u32, String>)
        -> HandlerResult<(MessageContext, ())> { Ok((context, ())) }

    async fn consume_message(&self, context: MessageContext, _: StreamContext<u32, u32, String>,
        _: Arc<Mutex<()>>, value: Payload<u32>, sender: &dyn Sender<u32>, result: ResultContext) -> HandlerResult {
...
async fn close_starts_before_active_handler_returns_but_response_waits() {
    let gates = Arc::new(Gates {
        after_done: Semaphore::new(0), release_handler: Semaphore::new(0),
        close_started: Semaphore::new(0), ended: Semaphore::new(0), responses: AtomicUsize::new(0),
    });
    ...
    environment.publish_runtime_config(Arc::new(RuntimeConfig::from_parts(
        ...
        [GrpcDataConnectorConfig { id: 4, name: "remote".to_owned(),
            address: "http://remote".to_owned(), connections_count: 1 }.into()],
        [GrpcEndpointConfig { id: 3, name: "request".to_owned(), id_data_connector: 4,
            tracing_enabled: false, grpc_method_type: GrpcMethodType::ClientStreaming }.into()], [],
    ).unwrap()));
    ...
    let call = tokio::spawn(async move {
        source.emit(MessageContext::new().with_stream_id("close-order"), Payload::new(1_u32)).await;
    });
    ...
    let closed_before_release = match started {
        Ok(permit) => { permit.unwrap().forget(); true }
        Err(_) => false,
    };
    ...
    tokio::time::timeout(Duration::from_secs(2), gates.ended.acquire())
        .await.unwrap().unwrap().forget();
    assert!(closed_before_release, "CloseAndRecv is delayed by active ConsumeMessage");
    assert_eq!(responses_before_release, 0, "HandleResponse must wait for ConsumeMessage");
